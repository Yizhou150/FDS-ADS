...
/*
 *Description：This procedure is find the level-order of the binary search tree we generated
 *Parameter[in] root the root of the binary tree
 */

void GenerateLevelOrder(Tree root) {
	Tree q[110];
	int h=0,t=0;
	q[h]=root;
	while (h<=t) {
		level[M++]=q[h]->Data; //fetch the value of the front of the queue as the next value in level-order
		if (q[h]->Left!=NULL) q[++t]=q[h]->Left;  //enque its left-child
		if (q[h]->Right!=NULL) q[++t]=q[h]->Right; //enque its right-child
		h++; //deque the front element
	}
} 
...
/*
 *Description：This procedure is to output the level-order of the generated tree for answer.c to output the correct answer
 *Parameter[in] n the number of nodes of the tree
 */

void OutputOutputData(int n) {
	int i;
	for (i=1;i<=n;i++) {
		printf("%d",level[i]); 
		if (i!=n) printf(" ");
...
int main(void) {
	...
	int n,i,k,temp;
	int NUMBER_OF_NODES,ITERATIONS;
	...
	/* generate the level-order of the tree */
	GenerateLevelOrder(root);


	/* output the input data of the test program*/
	OutputInputData(n);
